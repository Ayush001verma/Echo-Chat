...
export const addMembers = async (req, res) => {
  try {
    ...
    if (!isMember) return res.status(403).json({ message: "Only group members can add new members" });

    // Prevents duplicate memberships.
    const currentMemberIds = group.members.map(id => id.toString());
    const usersToAdd = newMembers.filter(id => !currentMemberIds.includes(id.toString()));
    ...
    group.members.forEach((member) => {
      const socketId = getReceiverSocketId(member._id.toString());
      if (socketId) {
        if (usersToAdd.includes(member._id.toString())) {
